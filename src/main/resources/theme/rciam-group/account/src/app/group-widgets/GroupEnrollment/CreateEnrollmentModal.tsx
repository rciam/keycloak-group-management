...
export const EnrollmentModal: FC<any> = (props) => {
  ...
  const validateEnrollment = () => {
    ...
    !(enrollment?.name?.length > 0) &&
      (errors.name = Msg.localize("requredFormError"));
    enrollment?.aup?.url?.length > 0 &&
      !reg_url.test(enrollment.aup.url) &&
      (errors.aup_url = Msg.localize("invalidUrlFormError"));
    !(enrollment?.aup?.url?.length > 0) &&
      props.validationRules?.aupEntity?.required === true &&
      (errors.aup_url = Msg.localize("requredFormError"));
    ...
    enrollment?.membershipExpirationDays &&
      !(enrollment?.membershipExpirationDays > 0) &&
      (errors.membershipExpirationDays = Msg.localize(
        "expirationDaysPositiveFormError"
      ));
    typeof enrollment?.membershipExpirationDays !== "number" &&
      (errors.membershipExpirationDays = Msg.localize(
        "expirationDaysNumberFormError"
      ));
    enrollment?.commentsNeeded &&
      (!enrollment?.commentsLabel || enrollment?.commentsLabel.length < 1) &&
      (errors.commentsLabel = Msg.localize("requredFormError"));
    enrollment?.commentsNeeded &&
      (!enrollment?.commentsDescription ||
        enrollment?.commentsDescription.length < 1) &&
      (errors.commentsDescription = Msg.localize("requredFormError"));
    if (enrollment?.validFrom) {
      ...
      if (parsedDate instanceof Date && isFinite(parsedDate.getTime())) {
        isPastDate(parsedDate) &&
          ...
          (errors.validFrom = Msg.localize("pastDateError"));
      } else {
        !(parsedDate instanceof Date && isFinite(parsedDate.getTime())) &&
          (errors.validFrom = Msg.localize("validFromInvalidFormError"));
      }
    }
    if (Object.keys(props.validationRules).length !== 0) {
      for (const field in props.validationRules) {
        field === "validFrom" &&
          ...
          (errors.validFrom = Msg.localize("validFromRequiredFormError"));
        props.validationRules[field]?.max &&
          ...
          (errors[field] =
            Msg.localize("fieldMaxFormError") +
            " (" +
            props.validationRules[field]?.max +
            ")");
    ...
    groupsService!
      ...
      .then((response: HttpResponse<any>) => {
        if (response.status === 200 || response.status === 204) {
          ...
          ContentAlert.danger(Msg.localize("deleteEnrollmentError"),[
            getError(response),
          ]);
        }
      })
        ...
        header={
          <React.Fragment>
            <h1 className="pf-c-modal-box__title gm_modal-title gm_flex-center">
              {enrollment?.id
                ? Msg.localize("enrollmentConfigurationModalTitleEdit")
                : Msg.localize("enrollmentConfigurationModalTitleCreate")}
              {enrollment?.id && (
                <Tooltip
                  content={
                    <div>
                  ...
                  <TrashIcon
                    ...
                    onClick={() => {
                      setModalInfo({
                        variant: "medium",
                        title: Msg.localize(
                          "deleteEnrollmentConfirmationTitle"
                        ),
                        message: Msg.localize(
                          "deleteEnrollmentConfirmationMessage"
                        ),
          ...
          <Form>
            ...
            >
              <Tooltip
                ...
                content={
                  <div>
                    <Msg msgKey="enrollmentConfigurationExpirationSwitchDisabledTooltip" />
                  </div>
                }
              >
                <Switch
            ...
            <FormGroup
              ...
              helperTextInvalid={
                touched.validFrom && !enrollment?.validFrom && errors.validFrom
              }
              validated={
                errors.validFrom && touched.validFrom ? "error" : "default"
              }
              labelIcon={
                <Popover
                  bodyContent={
            ...
            <FormFieldGroup
              header={
                <FormFieldGroupHeader
                  titleText={{
                    text: Msg.localize(
                      "enrollmentConfigurationCommentSectionTitle"
                    ),
            ...
            >
              <FormGroup
                label={Msg.localize(
                  "enrollmentConfigurationCommentEnableLabel"
                )}
                isRequired
                fieldId="simple-form-name-01"
                labelIcon={
              ...
              {enrollment.commentsNeeded ? (
                <React.Fragment>
                  <FormGroup
                    label={Msg.localize(
                      "enrollmentConfigurationCommentLabelLabel"
                    )}
                    isRequired
                    fieldId="simple-form-name-01"
                    helperTextInvalid={
                      touched.commentsLabel && errors.commentsLabel
                    }
                    validated={
                  ...
                  <FormGroup
                    label={Msg.localize(
                      "enrollmentConfigurationCommentDescriptionLabel"
                    )}
                    isRequired
                    fieldId="simple-form-name-01"
                    helperTextInvalid={
                      touched.commentsDescription && errors.commentsDescription
                    }
                    validated={
                      errors.commentsDescription && touched.commentsDescription
                        ? "error"
                        : "default"
                    }
                    onBlur={() => {
            ...
            <FormFieldGroup
              header={
                <FormFieldGroupHeader
                  titleText={{
                    text: Msg.localize("enrollmentConfigurationAupTitle"),
                    id: "field-group4-non-expandable-titleText-id",
                  }}
                />
              }
            ...
            <FormFieldGroup
              header={
                <FormFieldGroupHeader
                  titleText={{
                    text: Msg.localize(
                      "enrollmentConfigurationGroupRolesTitle"
                    ),
                    id: "field-group4-non-expandable-titleText-id",
                  }}
                />
            ...
            >
              <FormGroup
                ...
                validated={
                  errors.groupRoles && touched.groupRoles ? "error" : "default"
                }
              >
                <table className="gm_roles-table">
                  <tbody>
                    {props.groupRoles &&
                      Object.keys(props.groupRoles).map((role, index) => {
                        return (
                          <tr
                            onClick={() => {
                              roleHandler(role);
                          ...
                          >
                            ...
                            <td>
                              <Checkbox
                                ...
                                checked={enrollment?.groupRoles?.includes(role)}
