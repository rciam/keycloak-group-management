...
import { UserInfoModal } from "../Modals"; // Import the new modal
    ...
    groupsService!
      ...
      .then((response: HttpResponse<any>) => {
        ...
        if (response.status === 200 || response.status === 204) {
          ...
          ContentAlert.success(Msg.localize("suspendMembershipError"), [
            getError(response),
          ]);
        }
      });
    ...
    groupsService!
      ...
      .then((response: HttpResponse<any>) => {
        ...
        if (response.status === 200 || response.status === 204) {
          ...
          ContentAlert.success(Msg.localize("activateMembershipError"), [
            getError(response),
          ]);
        }
      });
    ...
    <>
      ...
      >
        <div>
          <p>
            {user.status === "ENABLED"
              ? Msg.localize("adminGroupMemberSuspendConfirmation")
              : Msg.localize("adminGroupMemberRevokeSuspendConfirmation")}
          </p>
          <Form>
            <FormGroup
              labelIcon={
                <Popover
                  bodyContent={
                    <div>
                      {user.status === "ENABLED"
                        ? Msg.localize("adminGroupMemberSuspendJustification")
                        : Msg.localize(
                            "adminGroupMemberRevokeSuspendJustification"
                          )}
                    </div>
                  }
                >
              ...
              label={Msg.localize("justificationLabel")}
              fieldId={""}
            >
              <TextArea
                type="text"
...
export const GroupMembers: FC<any> = (props) => {
  ...
  const [isKebabOpen, setIsKebabOpen] = useState<string>("");

  const onKebabToggle = (isOpen: boolean, memberId) => {
    console.log("Is open: " + isOpen ? memberId : "");
    setIsKebabOpen(isOpen ? memberId : "");
  };
    ...
    groupsService!
      ...
      .then((response: HttpResponse<any>) => {
        if (response.status === 200 || response.status === 204) {
          ...
          setLoading(false);
          ContentAlert.danger(Msg.localize("deleteMemberError"), [
            getError(response),
          ]);
        }
      });
    ...
    groupsService!
      ...
      .then((response: HttpResponse<any>) => {
        ...
        if (response.status === 200 || response.status === 204) {
          ...
          ContentAlert.danger(Msg.localize("activateMembershipError"), [
            getError(response),
          ]);
        }
      })
          ...
          <DataListItemCells
            dataListCells={[
              <DataListCell key="empty">
                <strong>
                  <Msg msgKey="adminGroupNoMembers" />
                </strong>
              </DataListCell>,
            ]}
          />
  ...
  const openUserInfoModal = (membership) => {
  ...
  const kebabItems = (member: any) => {
    const items = [
      ...
      >
        {Msg.localize("viewDetailsMembershipInformation")}
      </DropdownItem>,
      props.isGroupAdmin && (
        <DropdownItem
          key="remove-member"
        ...
        >
          {Msg.localize("adminGroupMemberRemove")}
        </DropdownItem>
      ),
      props.isGroupAdmin && member.status !== "SUSPENDED" && (
        <DropdownItem
          key="edit-membership"
          component="button"
          onClick={() => setEditMembership(member)}
        >
          {Msg.localize("editMembership")}
        </DropdownItem>
      ),
      props.isGroupAdmin && (
        <DropdownItem
        ...
        >
          {Msg.localize(
            member.status === "ENABLED"
              ? "adminGroupMemberSuspendTooltip"
              : "adminGroupMemberActivateTooltip"
          )}
        </DropdownItem>
      ),
    ];
  ...
  return (
    <React.Fragment>
      ...
      <TableActionBar
          ...
          <React.Fragment>
            <Tooltip
              ...
              content={
                <div>
                  {directMembers
                    ? Msg.localize("directMembersTooltip")
                    : Msg.localize("indirectMembersTooltip")}
                </div>
              }
            >
              <Checkbox
                className="gm_direct-member-checkbox"
                label={Msg.localize("adminGroupViewAllMembersButton")}
        ...
        searchText={Msg.localize("adminGroupSearchMember")}
        cancelText={Msg.localize("adminGroupSearchCancel")}
        search={(searchString) => {
            ...
            <DataListItemCells
              dataListCells={[
                ...
                >
                  ...
                  >
                    <strong>
                      <Msg msgKey="adminGroupMemberCellMembershipExp" />
                    </strong>
                ...
                ...(!directMembers
                  ? [
                      ...
                      >
                        <strong>
                          <Msg msgKey="adminGroupDirectMembership" />
                        </strong>
                      </DataListCell>,
                    ]
                  : []),
                        ...
                        <Tooltip
                          content={
                            <div>
                              {member.status === "ENABLED"
                                ? Msg.localize(
                                    "adminGroupMemberUserActiveTooltip"
                                  )
                                : member.status === "SUSPENDED"
                                ? Msg.localize(
                                    "adminGroupMemberUserSuspendedTooltip"
                                  )
                                : member.status === "PENDING"
                                ? Msg.localize(
                                    "adminGroupMemberUserPendingTooltip"
                                  )
                                : ""}
                            </div>
                          }
                            ...
                            <DataListCell width={2} key="secondary content6">
                              <Tooltip
                                content={
                                  <div>
                                    {member.direct
                                      ? Msg.localize("adminGroupIsDirect")
                                      : Msg.localize("adminGroupIsNotDirect")}
                                  </div>
                                }
                              >
                                <Checkbox
                    ...
                    <div className="gm_actions_container">
                      ...
                      >
                        <Button
                          isSmall
                          variant="primary"
                          className="gm_small_icon_button gm_profile-button"
                          onClick={() => openUserInfoModal(member)}
                        >
                          <UserIcon />
                        </Button>
                        ...
                        <React.Fragment>
                          ...
                          <Tooltip
                            content={
                              <div>
                                {member.status === "SUSPENDED"
                                  ? Msg.localize("editMembershipDisabled")
                                  : Msg.localize("editMembership")}
                              </div>
                            }
                          >
                      ...
                      {props.isGroupAdmin ? (
                        <Tooltip
                          content={
                            <div>
                              {member.status === "ENABLED"
                                ? Msg.localize("adminGroupMemberSuspendTooltip")
                                : member.status === "SUSPENDED"
                                ? Msg.localize(
                                    "adminGroupMemberActivateTooltip"
                                  )
                                : Msg.localize(
                                    "adminGroupMemberActivatePendingTooltip"
                                  )}
                            </div>
                          }
                        >
