...
export function calculateNewMembershipExpiresAt(oldValidFrom, oldMembershipExpiresAt) {
  ...
  const expiresAtDate = dateParse(oldMembershipExpiresAt); // Calculate duration in days

  const durationMs = expiresAtDate - validFromDate;
  const durationDays = Math.round(durationMs / (1000 * 60 * 60 * 24)); // Get today's date as a Date object

  const today = dateParse(getCurrentDate()); // Add duration to today

  const newExpiresAt = new Date(today);
  newExpiresAt.setDate(today.getDate() + durationDays); // Format as YYYY-MM-DD
