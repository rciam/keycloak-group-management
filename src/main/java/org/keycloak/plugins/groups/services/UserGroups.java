...
public class UserGroups {
    ...
    public Response createEnrollmentRequest(GroupEnrollmentRepresentation rep) {
        ...
        //active only member???
        UserGroupMembershipExtensionEntity member = userGroupMembershipExtensionRepository.getActiveByUserAndGroup(configuration.getGroup().getId(), user.getId());
        if (member != null)
    ...
    public Response acceptInvitation(@PathParam("id") String id) {
        ...
        if (userGroupMembershipExtensionRepository.getActiveByUserAndGroup(invitationEntity.getGroupEnrollmentConfiguration().getGroup().getId(), user.getId()) != null){
            throw new BadRequestException("You are already member of this group");
        }
